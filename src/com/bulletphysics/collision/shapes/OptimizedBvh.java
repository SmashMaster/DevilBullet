...
public class OptimizedBvh implements Serializable {
	...
	private final Vec3 bvhAabbMin = new Vec3();
	private final Vec3 bvhAabbMax = new Vec3();
	private final Vec3 bvhQuantization = new Vec3();
	...
	public void setInternalNodeAabbMin(int nodeIndex, Vec3 aabbMin) {
		if (useQuantization) {
	...
	public void setInternalNodeAabbMax(int nodeIndex, Vec3 aabbMax) {
		if (useQuantization) {
	...
	public Vec3 getAabbMin(int nodeIndex) {
		if (useQuantization) {
			Vec3 tmp = new Vec3();
	...
	public Vec3 getAabbMax(int nodeIndex) {
		if (useQuantization) {
			Vec3 tmp = new Vec3();
	...
	public void setQuantizationValues(Vec3 aabbMin, Vec3 aabbMax) {
		setQuantizationValues(aabbMin, aabbMax, 1f);
	}
	
	public void setQuantizationValues(Vec3 aabbMin, Vec3 aabbMax, float quantizationMargin) {
	...
	public void mergeInternalNodeAabb(int nodeIndex, Vec3 newAabbMin, Vec3 newAabbMax) {
		if (useQuantization) {
	...
	private static class NodeTriangleCallback extends InternalTriangleIndexCallback {
		...
		private final Vec3 aabbMin = new Vec3(), aabbMax = new Vec3();
	...
	public void build(StridingMeshInterface triangles, boolean useQuantizedAabbCompression, Vec3 _aabbMin, Vec3 _aabbMax) {
	...
	public void refitPartial(StridingMeshInterface meshInterface, Vec3 aabbMin, Vec3 aabbMax) {
...
//		}
	...
	public void updateBvhNodes(StridingMeshInterface meshInterface, int firstNode, int endNode, int index) {
		...
		Vec3[] triangleVerts/*[3]*/ = new Vec3[] { new Vec3(), new Vec3(), new Vec3() };
		Vec3 aabbMin = new Vec3(), aabbMax = new Vec3();
		Vec3 meshScaling = meshInterface.getScaling(new Vec3());
	...
	public void reportAabbOverlappingNodex(NodeOverlapCallback nodeCallback, Vec3 aabbMin, Vec3 aabbMax) {
...
//					break;
	...
	protected void walkStacklessTree(NodeOverlapCallback nodeCallback, Vec3 aabbMin, Vec3 aabbMax) {
		assert (!useQuantization);
	...
	protected void walkStacklessQuantizedTreeAgainstRay(NodeOverlapCallback nodeCallback, Vec3 raySource, Vec3 rayTarget, Vec3 aabbMin, Vec3 aabbMax, int startNodeIndex, int endNodeIndex) {
		assert (useQuantization);
...
//		boolean sign_z = rayDirection.z < 0f;
		...
		Vec3 rayAabbMin = new Vec3(raySource);
		Vec3 rayAabbMax = new Vec3(raySource);
		...
		Vec3 bounds_0 = new Vec3();
		Vec3 bounds_1 = new Vec3();
	...
	public void reportRayOverlappingNodex(NodeOverlapCallback nodeCallback, Vec3 raySource, Vec3 rayTarget) {
		...
		if (fast_path) {
			...
			Vec3 aabbMin = new Vec3(raySource);
			Vec3 aabbMax = new Vec3(raySource);
	...
	public void reportBoxCastOverlappingNodex(NodeOverlapCallback nodeCallback, Vec3 raySource, Vec3 rayTarget, Vec3 aabbMin, Vec3 aabbMax) {
		boolean fast_path = useQuantization && traversalMode == TraversalMode.STACKLESS;
		if (fast_path) {
			...
			Vec3 qaabbMin = new Vec3(raySource);
			Vec3 qaabbMax = new Vec3(raySource);
	...
	public long quantizeWithClamp(Vec3 point) {
		assert (useQuantization);
	...
	public void unQuantize(Vec3 vecOut, long vecIn) {

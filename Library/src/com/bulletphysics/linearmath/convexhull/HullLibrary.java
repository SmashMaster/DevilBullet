...
public class HullLibrary {
	...
	public boolean createConvexHull(HullDesc desc, HullResult result) {
		...
		ObjectArrayList<Vec3> vertexSource = new ObjectArrayList<Vec3>();
		MiscUtil.resize(vertexSource, vcount, Vec3.class);
		...
		if (ok) {
			//		if ( 1 ) // scale vertices back to their original size.
			{
				for (int i=0; i<ovcount[0]; i++) {
					Vec3 v = vertexSource.getQuick(i);
			...
			if (ok) {
				...
				ObjectArrayList<Vec3> vertexScratch = new ObjectArrayList<Vec3>();
				MiscUtil.resize(vertexScratch, hr.vcount, Vec3.class);
				...
				if (desc.hasHullFlag(HullFlags.TRIANGLES)) { // if he wants the results as triangle!
					...
					MiscUtil.resize(result.outputVertices, ovcount[0], Vec3.class);
	...
	private boolean computeHull(int vcount, ObjectArrayList<Vec3> vertices, PHullResult result, int vlimit) {
	...
	private int calchull(ObjectArrayList<Vec3> verts, int verts_count, IntArrayList tris_out, int[] tris_count, int vlimit) {
	...
	private int calchullgen(ObjectArrayList<Vec3> verts, int verts_count, int vlimit) {
		...
		//int j;
		Vec3 bmin = new Vec3(verts.getQuick(0));
		Vec3 bmax = new Vec3(verts.getQuick(0));
		...
		for (int j=0; j<tris.size(); j++) {
			...
			VectorUtil.sub(tmp, verts.getQuick(t.vmax), verts.getQuick(t.getCoord(0)));
			t.rise = n.dot(tmp);
		}
		Tri te;
		vlimit -= 4;
		while (vlimit > 0 && ((te = extrudable(epsilon)) != null)) {
			...
			while ((j--) != 0) {
				...
				VectorUtil.sub(tmp1, verts.getQuick(nt.getCoord(1)), verts.getQuick(nt.getCoord(0)));
				VectorUtil.sub(tmp2, verts.getQuick(nt.getCoord(2)), verts.getQuick(nt.getCoord(1)));
				VectorUtil.cross(tmp, tmp1, tmp2);
	...
	private Int4 findSimplex(ObjectArrayList<Vec3> verts, int verts_count, IntArrayList allow, Int4 out) {
		Vec3 tmp = new Vec3();
		Vec3 tmp1 = new Vec3();
		Vec3 tmp2 = new Vec3();

		Vec3[] basis = new Vec3[/*3*/] { new Vec3(), new Vec3(), new Vec3() };
		basis[0].set(0.01f, 0.02f, 1.0f);
		int p0 = maxdirsterid(verts, verts_count, basis[0], allow);
		VectorUtil.negate(tmp, basis[0]);
		int p1 = maxdirsterid(verts, verts_count, tmp, allow);
		VectorUtil.sub(basis[0], verts.getQuick(p0), verts.getQuick(p1));
		if (p0 == p1 || (basis[0].x == 0f && basis[0].y == 0f && basis[0].z == 0f)) {
			out.set(-1, -1, -1, -1);
			return out;
		}
		tmp.set(1f, 0.02f, 0f);
		VectorUtil.cross(basis[1], tmp, basis[0]);
		tmp.set(-0.02f, 1f, 0f);
		VectorUtil.cross(basis[2], tmp, basis[0]);
		...
		VectorUtil.sub(basis[1], verts.getQuick(p2), verts.getQuick(p0));
		VectorUtil.cross(basis[2], basis[1], basis[0]);
		basis[2].normalize();
		int p3 = maxdirsterid(verts, verts_count, basis[2], allow);
		if (p3 == p0 || p3 == p1 || p3 == p2) {
			VectorUtil.negate(tmp, basis[2]);
		...
		VectorUtil.sub(tmp1, verts.getQuick(p1), verts.getQuick(p0));
		VectorUtil.sub(tmp2, verts.getQuick(p2), verts.getQuick(p0));
		VectorUtil.cross(tmp2, tmp1, tmp2);
		VectorUtil.sub(tmp1, verts.getQuick(p3), verts.getQuick(p0));
		if (tmp1.dot(tmp2) < 0) {
			int swap_tmp = p2;
			p2 = p3;
			p3 = swap_tmp;
	...
	private void bringOutYourDead(ObjectArrayList<Vec3> verts, int vcount, ObjectArrayList<Vec3> overts, int[] ocount, IntArrayList indices, int indexcount) {
	...
	private boolean cleanupVertices(int svcount,
			ObjectArrayList<Vec3> svertices,
			int stride,
			int[] vcount, // output number of vertices
			ObjectArrayList<Vec3> vertices, // location to store the results.
			float normalepsilon,
			Vec3 scale) {
		...
		ObjectArrayList<Vec3> vtx_ptr = svertices;
		int vtx_idx = 0;

		//	if ( 1 )
		{
			for (int i=0; i<svcount; i++) {
				Vec3 p = vtx_ptr.getQuick(vtx_idx);
		...
		for (int i=0; i<svcount; i++) {
			...
			//		if ( 1 )
			{
				...
				if (j == vcount[0]) {
					Vec3 dest = vertices.getQuick(vcount[0]);
					dest.x = px;
					dest.y = py;
					dest.z = pz;
					vcount[0]++;
	...
	private static Vec3 orth(Vec3 v, Vec3 out) {
		Vec3 a = new Vec3();
		a.set(0f, 0f, 1f);
		VectorUtil.cross(a, v, a);

		Vec3 b = new Vec3();
		b.set(0f, 1f, 0f);
		VectorUtil.cross(b, v, b);

		if (a.length() > b.length()) {
			VectorUtil.normalize(out, a);
			return out;
		}
		else {
			VectorUtil.normalize(out, b);
			return out;
		}
	}
	
	private static int maxdirfiltered(ObjectArrayList<Vec3> p, int count, Vec3 dir, IntArrayList allow) {
		assert (count != 0);
		int m = -1;
	...
	private static int maxdirsterid(ObjectArrayList<Vec3> p, int count, Vec3 dir, IntArrayList allow) {
		Vec3 tmp = new Vec3();
		Vec3 tmp1 = new Vec3();
		Vec3 tmp2 = new Vec3();
		Vec3 u = new Vec3();
		...
		while (m == -1) {
			...
			for (float x = 0f; x <= 360f; x += 45f) {
				...
				VectorUtil.scale(tmp1, s, u);
				VectorUtil.scale(tmp2, c, v);
	...
	private static Vec3 triNormal(Vec3 v0, Vec3 v1, Vec3 v2, Vec3 out) {
		Vec3 tmp1 = new Vec3();
		Vec3 tmp2 = new Vec3();

		// return the normal of the triangle
		// inscribed by v0, v1, and v2
		VectorUtil.sub(tmp1, v1, v0);
		VectorUtil.sub(tmp2, v2, v1);
		Vec3 cp = new Vec3();
		VectorUtil.cross(cp, tmp1, tmp2);
		float m = cp.length();
		if (m == 0) {
			out.set(1f, 0f, 0f);
			return out;
		}
		VectorUtil.scale(out, 1f / m, cp);
		return out;
	}
	
	private static boolean above(ObjectArrayList<Vec3> vertices, Int3 t, Vec3 p, float epsilon) {
		Vec3 n = triNormal(vertices.getQuick(t.getCoord(0)), vertices.getQuick(t.getCoord(1)), vertices.getQuick(t.getCoord(2)), new Vec3());
		Vec3 tmp = new Vec3();
		VectorUtil.sub(tmp, p, vertices.getQuick(t.getCoord(0)));
	...
	private static void addPoint(int[] vcount, ObjectArrayList<Vec3> p, float x, float y, float z) {
		// XXX, might be broken
		Vec3 dest = p.getQuick(vcount[0]);
		dest.x = x;
		dest.y = y;
		dest.z = z;
		vcount[0]++;
	...
	private static float getDist(float px, float py, float pz, Vec3 p2) {
		float dx = px - p2.x;
		float dy = py - p2.y;
		float dz = pz - p2.z;
